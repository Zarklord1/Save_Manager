...
void GuiButton::setHitbox(f32 w, f32 h) {
	widthHitbox = w;
	heightHitbox = h;
}

void GuiButton::setHitboxStart(f32 x, f32 y) {
	xOffsetHitbox = x;
	yOffsetHitbox = y;
}
...
bool GuiButton::isInside(f32 x, f32 y) {
	return (	x > (this->getHitboxCenterX() - getScaleX() * widthHitbox * 0.5f)
			&&	x < (this->getHitboxCenterX() + getScaleX() * widthHitbox * 0.5f)
			&&	y > (this->getHitboxCenterY() - getScaleY() * heightHitbox * 0.5f)
			&&	y < (this->getHitboxCenterY() + getScaleY() * heightHitbox * 0.5f));

}


f32 GuiButton::getHitboxCenterX() {
    f32 pCenterX = 0.0f;

    if(parentElement)
        pCenterX = parentElement->getCenterX();

    pCenterX += xOffsetHitbox + xoffsetDyn;
    ...
    if(alignment & ALIGN_LEFT)
    {
        ...
        pCenterX -= pWidth * 0.5f * pScale - widthHitbox * 0.5f * getScaleX();
    }
...
f32 GuiButton::getHitboxCenterY() {
    f32 pCenterY = 0.0f;

    if(parentElement)
        pCenterY = parentElement->getCenterY();

    pCenterY += yOffsetHitbox + yoffsetDyn;
    ...
    if(alignment & ALIGN_TOP)
    {
        ...
        pCenterY += pHeight * 0.5f * pScale - heightHitbox * 0.5f * getScaleY();
    }
