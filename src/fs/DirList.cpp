...
u64 DirList::GetFoldersize(std::string folderpath)
{
	u64 totalSize = 0;
	if(folderpath.size() < 3)
		return false;
	...
	while ((dirent = readdir(dir)) != 0) {
		...
		if(isDir) {
			if(strcmp(filename,".") == 0 || strcmp(filename,"..") == 0)
				continue;
			std::string temp = folderpath + "/" + filename;
			totalSize = totalSize + GetFoldersize(temp);
		} else {
			std::string temp = folderpath + "/" + filename;
			totalSize = totalSize + InternalGetFilesize(temp);
		}
	}
	closedir(dir);
...
u64 DirList::InternalGetFilesize(std::string filepath)
{
	struct stat st;
